...
public class BoatComponent extends Component implements ActionListener
{
  ...
  public void initBoat(){
    int newSize = 75; // Ukuran baru untuk elemen kapal
    int wallBottomY = Constants.WALL_BOTTOM_Y; // Posisi Y dinding bawah
    int boatYPosition = wallBottomY - newSize - 15; // Atur posisi Y kapal agar berada tepat di atas dinding

    // Pastikan boatYPosition tidak negatif
    if (boatYPosition < 0) {
        boatYPosition = 0; 
    }

    System.out.println("Boat Position: (" + this.getX() + ", " + boatYPosition + ")");
    System.out.println("Boat Size: " + newSize);

    boatElements.add(new BoatElement(this.getX(), boatYPosition, newSize, newSize));
  }
